...
// ── TOKEN REGISTRY ────────────────────────────────────────────────────────────
// Tier 1 — highest liquidity, always eligible in EXPANSION
const TIER1_TOKENS = {
...
// Tier 2 — mid liquidity, eligible in EXPANSION + EXPANSION_CONFIRMED
const TIER2_TOKENS = {
...
// Tier 3 — smaller ecosystem, only EXPANSION_CONFIRMED with high conviction
const TIER3_TOKENS = {
  POPCAT: '7GCihgDB8fe6KNjn2MYtkzZcRjQy3t9GHdC8uHYmW2hr',
  MEW:    'MEW1gQWJ3nEXg2qgERiKu7FAFj79PHvQVREQUzScPP5',
  ZEUS:   'ZEUS1aR7aX8pfVeGg7dmmoiXAzs1TcwHy4XeEH43Lz5',
  KMNO:   'KMNo3nJsBXfcpJTVhZcXLW7RmTwTt4GVFE7suUBo9sS',
};

// Token metadata — styles, volatility profile, min conditions
const TOKEN_META = {
  BONK:   { styles:['momentum','breakout','swing','high_beta'], vol:'HIGH',   minRegime:'EXPANSION' },
  WIF:    { styles:['momentum','breakout','swing','high_beta'], vol:'HIGH',   minRegime:'EXPANSION' },
  JTO:    { styles:['momentum','growth','defensive','pullback','mean_reversion_small','value','dividend'], vol:'MED', minRegime:'EXPANSION' },
  PYTH:   { styles:['growth','pullback','defensive','mean_reversion_small','value'], vol:'MED',  minRegime:'EXPANSION' },
  RAY:    { styles:['momentum','growth','mean_reversion','pullback','defensive','value','dividend'], vol:'MED', minRegime:'EXPANSION' },
  ORCA:   { styles:['growth','momentum','pullback','value'], vol:'MED',  minRegime:'EXPANSION' },
  DRIFT:  { styles:['momentum','breakout','growth'], vol:'HIGH', minRegime:'EXPANSION' },
  POPCAT: { styles:['momentum','breakout','swing','high_beta'], vol:'VHIGH', minRegime:'EXPANSION_CONFIRMED' },
  MEW:    { styles:['momentum','breakout','swing','high_beta'], vol:'VHIGH', minRegime:'EXPANSION_CONFIRMED' },
  ZEUS:   { styles:['momentum','growth','breakout'], vol:'HIGH', minRegime:'EXPANSION_CONFIRMED' },
  KMNO:   { styles:['momentum','growth','value'], vol:'HIGH', minRegime:'EXPANSION_CONFIRMED' },
};

// Size multipliers by volatility — higher vol = smaller position
...
// ── PICK TOKEN — scored selection across tiers ───────────────────────────────
async function pickToken(regime, scores, state, allowedStyles=[]) {
  const finalRegime = regime.dominant;
  const expProb     = regime.expProb || 0;
  const tradability = scores.tradability || 0.5;
  const confidence  = regime.confidence || 0.5;

  // Build candidate pool based on regime
  const candidates = [];

  // Tier 1 — always available in EXPANSION+
  Object.entries(TIER1_TOKENS).forEach(([sym, mint]) => {
    const meta = TOKEN_META[sym];
    candidates.push({ sym, mint, tier:1, meta });
  });

  // Tier 2 — available in EXPANSION with good tradability
  if(tradability >= 0.55) {
    Object.entries(TIER2_TOKENS).forEach(([sym, mint]) => {
      const meta = TOKEN_META[sym];
      candidates.push({ sym, mint, tier:2, meta });
    });
  }

  // Tier 3 — only EXPANSION_CONFIRMED with high confidence
  if(finalRegime === 'EXPANSION_CONFIRMED' && confidence >= 0.72 && expProb >= 0.75) {
    Object.entries(TIER3_TOKENS).forEach(([sym, mint]) => {
      const meta = TOKEN_META[sym];
      candidates.push({ sym, mint, tier:3, meta });
    });
  }

  // Filter by style compatibility
  const styleFiltered = candidates.filter(c => {
    if(!allowedStyles.length) return c.tier === 1; // no styles = tier 1 only
    return c.meta.styles.some(s => allowedStyles.includes(s));
  });

  const pool = styleFiltered.length > 0 ? styleFiltered : candidates.filter(c => c.tier === 1);

  // Score each candidate
  // Try to fetch recent prices for momentum scoring (best effort, non-blocking)
  const scored = await Promise.all(pool.map(async c => {
    let score = 0;

    // Tier preference — slight bias toward Tier 1 in uncertain conditions
    if(c.tier === 1) score += confidence >= 0.7 ? 5 : 15;
    else if(c.tier === 2) score += 10;
    else score += 5; // Tier 3 — riskier, lower base score

    // Style match quality — more styles matching = better fit
    const matchCount = allowedStyles.length
      ? c.meta.styles.filter(s => allowedStyles.includes(s)).length
      : 0;
    score += matchCount * 8;

    // Vol penalty — high vol tokens get smaller score in uncertain markets
    const volPenalty = { LOW:0, MED:0, HIGH: confidence<0.7?-10:0, VHIGH: confidence<0.8?-20:-5 };
    score += volPenalty[c.meta.vol] || 0;

    // Prefer momentum tokens when expansion is strong
    if(expProb >= 0.7 && c.meta.styles.includes('momentum')) score += 12;
    if(expProb >= 0.8 && c.meta.styles.includes('breakout')) score += 8;

    // Add small random jitter to avoid always picking same token
    score += Math.random() * 5;
  ...
  const volMult = VOL_SIZE_MULT[winner.meta.vol] || 1.0;

  console.log(`[PICK] ${winner.sym} (Tier ${winner.tier}) score=${winner.score.toFixed(1)} vol=${winner.meta.vol} mult=${volMult}`);
  console.log(`[PICK] Pool: ${scored.map(c=>`${c.sym}(${c.score.toFixed(0)})`).join(', ')}`);

  return { token:winner.sym, mint:winner.mint, tier:winner.tier, volMult, meta:winner.meta };
}
...
async function runCycle(state, walletAddress) {
  ...
  if(positionUpdate && positionUpdate.action !== 'HOLD') {
    ...
    // Pass allowed styles from state (set by InvestOS override in server.js)
    const allowedStyles = state.currentAllowedStyles || [];
    const tokenInfo = await pickToken(regime, scores, state, allowedStyles);

    // Size with vol multiplier — high vol tokens get smaller position
    const baseSOL = (walletBalance || state.totalSOL || 0.5) * alloc.oppPct * regime.expProb;
    const swapSOL = +(baseSOL * (tokenInfo.volMult || 1.0)).toFixed(4);

    const liquidity = await fetchTokenLiquidity(tokenInfo.mint, swapSOL);

    if(!liquidity.tradable) {
      result.action = 'BLOCKED';
      result.blockReason = `${tokenInfo.token} price impact ${(liquidity.priceImpact*100).toFixed(2)}% — too illiquid`;
    } else {
      ...
      result.proposal = {
        ...
        volProfile:  tokenInfo.meta?.vol || 'MED',
        volMult:     tokenInfo.volMult || 1.0
      };
    }
...
module.exports = { runCycle, fetchWalletBalance, fetchTokenPrice, TIER1_TOKENS, TIER2_TOKENS, TIER3_TOKENS, TOKEN_META, SOL_MINT };
