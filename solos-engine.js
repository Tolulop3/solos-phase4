...
async function fetchSOLPrice() {
  ...
  // Provider 1: CoinGecko simple price
  try {
    ...
    if(r.ok) {
      ...
      if(d?.solana?.usd) {
        console.log(`[PRICE] coingecko_simple: $${d.solana.usd}`);
        return { price:d.solana.usd, trend24h:(d.solana.usd_24h_change||0)/100, trend7d:0, source:'coingecko_simple' };
      }
      errors.push(`coingecko_simple: bad data ${JSON.stringify(d).slice(0,80)}`);
    } else {
      errors.push(`coingecko_simple: HTTP ${r.status}`);
    }
  } catch(e) { errors.push(`coingecko_simple: ${e.message}`); }

  // Provider 2: CoinGecko full
  try {
    const controller = new AbortController();
    ...
    if(r.ok) {
      ...
      if(m?.current_price?.usd) {
        console.log(`[PRICE] coingecko_full: $${m.current_price.usd}`);
        return { price:m.current_price.usd, trend24h:(m.price_change_percentage_24h||0)/100,
          trend7d:(m.price_change_percentage_7d||0)/100,
          volume24h:m.total_volume?.usd||0, marketCap:m.market_cap?.usd||1, source:'coingecko_full' };
      }
      errors.push(`coingecko_full: no market_data`);
    } else {
      errors.push(`coingecko_full: HTTP ${r.status}`);
    }
  } catch(e) { errors.push(`coingecko_full: ${e.message}`); }

  // Provider 3: Jupiter price
  try {
    const controller = new AbortController();
    ...
    if(r.ok) {
      ...
      if(p) {
        console.log(`[PRICE] jupiter: $${p}`);
        return { price:p, trend24h:0, trend7d:0, source:'jupiter' };
      }
      errors.push(`jupiter: no price in response`);
    } else {
      errors.push(`jupiter: HTTP ${r.status}`);
    }
  } catch(e) { errors.push(`jupiter: ${e.message}`); }

  // All failed — log each error
