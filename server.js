...
const TOKEN_STYLES = {
  JTO:  ['defensive','pullback','low_vol','momentum','mean_reversion_small','growth'],
  BONK: ['momentum','high_beta','breakout','swing'],
  WIF:  ['momentum','high_beta','breakout','swing'],
  PYTH: ['pullback','low_vol','defensive','mean_reversion_small','growth'],
  RAY:  ['pullback','momentum','mean_reversion','mean_reversion_small','defensive','growth']
};
...
async function executeCycle(manual=false) {
  ...
  try {
    ...
    // but InvestOS says RISK_ON → EXPANSION. Override and propose trade.
    const engineBlocked = ['STAKE','BLOCKED'].includes(result.action);
    if((finalRegime==='EXPANSION'||finalRegime==='EXPANSION_CONFIRMED') &&
       !state.openPosition &&
       engineBlocked &&
      ...
      const exposure = Math.min(override.exposure || 0.3, 0.3); // cap at 30%
      const swapSOL = +(walletBal * exposure * 0.22).toFixed(4); // 22% of exposed capital

      // Pick token from allowed styles — prefer momentum/breakout in EXPANSION
      const allowed = override.allowedStyles || [];
      const preferredOrder = ['BONK','WIF','JTO','RAY','PYTH'];
      const token = preferredOrder.find(tok =>
        (TOKEN_STYLES[tok]||[]).some(s => allowed.includes(s))
      ) || 'JTO';
      const mint = TIER1_TOKENS[token] || TIER2_TOKENS[token];
