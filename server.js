...
// ── STEP 3: PREVENT DUPLICATE INSTANCES ──────────────────────────────────────
if(global.__SOLOS_RUNNING__) {
...
// ── STEP 1: CLEAN BOT INIT ───────────────────────────────────────────────────
const bot = new TelegramBot(BOT_TOKEN, {
  polling: true,
  request: { timeout: 30000 }
});

// ── STEP 2: TELEGRAM SESSION RESET ───────────────────────────────────────────
(async () => {
  try {
    await bot.deleteWebHook();
    await bot.getMe();
    console.log('Telegram session reset complete');
  } catch(e) {
    console.log('Telegram init warning:', e.message);
  }
})();
...
async function syncToDashboard(state, lastResult) {
  ...
  try {
    ...
    const putRes = await fetch(`${JSONBIN_BASE}/${JSONBIN_BIN}`, {
      method: 'PUT',
      headers: {
    ...
    console.log('JSONBin sync status:', putRes.status);
    if(putRes.status !== 200) {
      const txt = await putRes.text();
      console.log('JSONBin sync error:', txt.slice(0,200));
    }
  } catch(e) {
...
bot.onText(/\/setup/, async () => {
  ...
  try {
    ...
    console.log('JSONBin setup response:', JSON.stringify(d));
    const binId = d?.metadata?.id;
    if(binId) {
      await send(`✅ JSONBin created!\n\nBin ID: \`${binId}\`\n\nNow add to Render environment:\nJSONBIN_BIN = ${binId}\n\nThen paste Bin ID into dashboard Phase 4 sync panel.`);
    } else {
      await send('❌ Failed to create bin: ' + JSON.stringify(d).slice(0,200));
    }
  } catch(e) {
...
// ── STEP 4: CRON WITH LOCK — prevents double execution ───────────────────────
