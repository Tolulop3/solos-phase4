...
const TOKEN_STYLES = {
  JTO:  ['defensive','pullback','low_vol','momentum','mean_reversion_small'],
  BONK: ['momentum','high_beta','breakout'],
  WIF:  ['momentum','high_beta','breakout'],
  PYTH: ['pullback','low_vol','defensive','mean_reversion_small'],
  RAY:  ['pullback','momentum','mean_reversion','mean_reversion_small','defensive']
};
...
function computeInvestOSDecision(inv) {
  ...
  // Score from 3 independent inputs
  let score = 0;
  ...
  // SOL conviction modifier
  // DESIGN DECISION: directional signal controls exposure sizing, NOT participation.
  // SHORT reduces exposure but does NOT suppress defensive/mean_reversion trades.
  // Style filter handles participation. Exposure score handles sizing.
  ...
  // Map score to regime + exposure + styles
  let regime, exposure, styles, note;

  if(score >= 3) {
    regime='RISK_ON'; exposure=1.0;
    styles=['momentum','breakout','high_beta','pullback'];
    note=`Score ${score} → full deployment`;
  } else if(score === 2) {
    regime='RISK_ON'; exposure=0.85;
    styles=['momentum','breakout','pullback','defensive'];
    note=`Score ${score} → strong deployment`;
  } else if(score === 1) {
    regime='NEUTRAL'; exposure=0.6;
    styles=['pullback','momentum','mean_reversion','defensive'];
    note=`Score ${score} → selective`;
  } else if(score === 0) {
    regime='DEFENSIVE'; exposure=0.4;
    styles=['pullback','defensive','low_vol','mean_reversion'];
    note=`Score ${score} → defensive`;
  } else if(score === -1) {
    regime='DEFENSIVE'; exposure=0.3;
    styles=['pullback','defensive','low_vol'];
    note=`Score ${score} → reduced`;
  } else if(score === -2) {
    regime='CAPITAL_PRESERVATION'; exposure=0.15;
    styles=['mean_reversion_small','defensive'];
    note=`Score ${score} → capital preservation`;
  } else {
    regime='STRESS'; exposure=0.0;
    styles=[];
    note=`Score ${score} → vault only`;
  }

  console.log(`InvestOS decision: regime=${regime} score=${score} exposure=${exposure} styles=[${styles.join(',')}] note=${note}`);
  return { regime, exposure, styles, score, note,
    raw:{ macro:inv.macroRegime, market:inv.marketRegime, health:inv.health,
...
async function fetchInvestOSMacro() {
  try {
    ...
    console.log(`InvestOS raw: macro=${m.macroRegime} market=${m.marketRegime} health=${m.health} SOL=${m.solDirection}@${m.solConviction}% age=${m.ageHours}h fresh=${m.fresh}`);
    return m;
...
function applyInvestOSOverride(localRegime, inv, scores) {
  ...
  if(investosRegime === 'STRESS') {
    ...
    // Check PROBE eligibility — only escape from CAPITAL_PRESERVATION
    ...
    // DEFENSIVE → NEUTRAL_DEGRADED in SolOS enum
    // Check shadow candidate
    if(localRegime==='EXPANSION' && inv &&
      ...
      finalRegime = 'NEUTRAL_DEGRADED'; // shadow candidate logged separately
    } else {
    ...
    // Both systems agree expansion
    if(localRegime === 'EXPANSION' && inv?.solConviction >= 65 && inv?.solDirection === 'LONG' && inv?.fresh) {
      finalRegime = 'EXPANSION_CONFIRMED';
    } else {
      finalRegime = localRegime === 'EXPANSION' ? 'EXPANSION' : 'NEUTRAL';
    }
  } else {
...
function checkExplorationEligible(inv, scores) {
  ...
  // Conviction threshold — relaxed from 68 to 55 (SOL SHORT still allowed cautiously)
  if(inv.solConviction < 55)
    return { eligible:false, reason:`SOL conviction ${inv.solConviction}% below 55% threshold` };

  // Sanity threshold — relaxed from 72 to 58
  if(!scores || scores.sanity < 0.58)
    return { eligible:false, reason:`Sanity ${scores?Math.round(scores.sanity*100):'?'} below 58 threshold` };

  // Volume proxy — relaxed from 70 to 60
  const volumeOk = scores.tradability >= 0.60;
  if(!volumeOk)
    return { eligible:false, reason:`Tradability ${Math.round(scores.tradability*100)} below 60`, whaleProxy:true };
...
function formatCycleAlert(result,inv,override) {
  ...
  return [
    ...
    override?.note?`💡 ${override.note.slice(0,60)}`:'',
    override?.allowedStyles?.length?`📋 Styles: ${override.allowedStyles.slice(0,3).join(', ')}${override.allowedStyles.length>3?'...':''} · exposure ${override.exposure!==undefined?Math.round(override.exposure*100)+'%':'—'}`:'',
...
async function executeCycle(manual=false) {
  ...
  try {
    ...
    // Block all trading if price data unavailable
    if(result.marketData?.tradingDisabled) {
      console.log('[CYCLE] Price unavailable — trading disabled this cycle');
      if(manual) await send(`⚠️ *Price data unavailable*\nAll providers failed. Trading disabled this cycle. Monitoring continues.`);
      state.history = state.history||[];
      state.history.push({ cycle:result.cycleNum, action:'PRICE_UNAVAILABLE',
        regime:'unknown', sanity:0, timestamp:new Date().toISOString() });
